...
function* loginSaga(action: any):any {
  try {
    ...
    yield put(actions.authFailure(e.response?.data?.message || 'Login failed'));
  }
}
...
function* registerSaga(action: any):any {
  try {
    ...
    yield put(actions.authFailure(e.response?.data?.message || 'Registration failed'));
  }
}
...
function* updateProfileSaga(action: any):any {
  try {
    ...
    yield put(actions.authFailure(e.response?.data?.message || 'Update failed'));
  }
}
